...
openssl req -x509 -newkey rsa:2048 -days 3650 -nodes   -keyout "${CERT_DIR}/server.key"   -out "${CERT_DIR}/server.crt"   -subj "/CN=*.${BASE_DOMAIN}/O=DTS"   -addext "subjectAltName=DNS:*.${BASE_DOMAIN},DNS:${BASE_DOMAIN}"
