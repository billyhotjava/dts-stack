...
generate_env_base(){
  ...
  # 统一的前端密码登录开关（两端共用）。true=显示账号密码；false=隐藏，仅证书登录
  : "${WEBAPP_PASSWORD_LOGIN_ENABLED:=true}"
  # 兼容旧变量：Vite 构建期读取（隐藏= true，显示= false）。从统一开关推导
  if [[ "${WEBAPP_PASSWORD_LOGIN_ENABLED}" =~ ^([Tt]rue|1|[Yy]es|on)$ ]]; then
    VITE_HIDE_PASSWORD_LOGIN="false"
  else
    VITE_HIDE_PASSWORD_LOGIN="true"
  fi
...
WEBAPP_PASSWORD_LOGIN_ENABLED=${WEBAPP_PASSWORD_LOGIN_ENABLED}
VITE_HIDE_PASSWORD_LOGIN=${VITE_HIDE_PASSWORD_LOGIN}
