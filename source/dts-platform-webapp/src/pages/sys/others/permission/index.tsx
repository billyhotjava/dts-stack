...
// Mock users removed with MSW; keep empty list to avoid build-time dependency
...
export default function PermissionPage() {
    ...
    const { check, checkAny, checkAll } = useAuthCheck();
    // Force-disable the mock account switcher.
    const enableMock = false;

    const handleSwitch = (_username: string) => {
        if (!enableMock) return;
        if (_username === username) return;
        const user = DB_USER.find((u) => u.username === _username);
        if (user) signIn({ username: user.username, password: user.password });
    };
    return (
