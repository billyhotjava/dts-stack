...
const filterNavData = (permissions: string[]) => {
	const source = GLOBAL_CONFIG.routerMode === "backend" ? getBackendNavData() : frontendNavData;
	return source
		.map((group) => {
			// 过滤组内的项目
...
export const useFilteredNavData = () => {
	...
	// subscribe to menu changes to recompute nav when backend menus load
	...
	const filteredNavData = useMemo(() => filterNavData(permissionCodes), [permissionCodes, menus]);
