...
- Client navigation is provided by static frontend routes; no menu-creation API is required.
  - Use `VITE_APP_ROUTER_MODE=frontend` (default).
- To hide the portal menu management UI (no menu creation via API):
  - Set `VITE_ENABLE_PORTAL_MENU_MGMT=false` (default). The sidebar entry and route are removed and related APIs are not called.
