...
export default function UserManagementView() {
	...
	const expandedRowRender = useCallback(
		(record: KeycloakUser) => {
			...
			const phone =
				getSingleAttr(attrs, "phone") || getSingleAttr(attrs, "mobile") || getSingleAttr(attrs, "telephone");
			...
			async function worker() {
				while (i < entries.length) {
					...
					const roles: KeycloakRole[] = await KeycloakUserService.getUserRoles(userId, { silent: true });
						...
						const names = filtered.map((r) => normalizeRoleCode(r?.name)).filter((code) => Boolean(code));
		...
		() => [
			...
			},
			{
				...
				render: (_, record) => {
					...
					const gp =
						Array.isArray((record as any).groups) && (record as any).groups.length ? (record as any).groups[0] : "";
			...
			},
			{
				...
				render: (_, record) => {
					...
					const fallback =
						getSingleAttr(record.attributes, "person_security_level") ||
						getSingleAttr(record.attributes, "person_level");
