package com.yuzhi.dts.platform.service.keycloak;
 ...
 * Mirrors the admin module service, scoped for the platform backend.
 */
@Service
public class KeycloakAuthService {
    ...
    private TokenResponse exchangeForTokens(String username, String password) {
        ...
        try {
            if (log.isDebugEnabled()) {
                log.debug("[kc] token exchange grant=password user={} client={} endpoint={} scope={}",
                    sanitize(username), clientId, tokenEndpoint, scopeParam);
            }
            ResponseEntity<TokenResponse> response = restTemplate.exchange(
                tokenEndpoint,
                HttpMethod.POST,
            ...
            if (log.isWarnEnabled()) {
                ...
                log.warn("[kc] token exchange failed status={} user={} client={} body={}",
                    ex.getStatusCode().value(), sanitize(username), clientId, truncate(resp, 256));
            }
            throw translateAuthError(ex);
        }
    }
    ...
    public void revokeRefreshToken(String refreshToken) {
        ...
        try {
            restTemplate.exchange(revokeEndpoint, HttpMethod.POST, new HttpEntity<>(form, headers), Void.class);
    ...
    private Map<String, Object> buildUserProfile(String fallbackUsername, Map<String, Object> userInfo, Map<String, Object> claims) {
        ...
        String username = firstNonBlank(user.get("preferred_username"), claims.get("preferred_username"), fallbackUsername);
        ...
        String fullName = firstNonBlank(claims.get("fullname"), user.get("fullname"), user.get("name"), claims.get("name"), givenName, username);
        ...
        // Collect known custom attributes from claims into a nested attributes map so FE can read them
    ...
    private List<String> extractRoles(Map<String, Object> claims) {
        ...
        // Include top-level roles claim when present (from Keycloak "roles" client scope)
        roles.addAll(toStringList(claims.get("roles")));
        ...
        // resource_access.{client}.roles for client roles
